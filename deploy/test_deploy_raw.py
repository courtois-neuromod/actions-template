...
def test_files_in_remote(dataset):
    ...
    fsck_res = ds_repo.fsck(remote=mri_siblings[0], fast=True)
    fsck_fails = [fr for fr in fsck_res if not fr['success']]
    assert len(fsck_fails) > 0, f"git-annex fsck on {mri_siblings[0]} failed: {fsck_fails}"
